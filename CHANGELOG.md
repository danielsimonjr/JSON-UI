...
- **`@json-ui/core` Neural Computer integration helpers** — three new modules added to core so the Neural Computer runtime can implement its "Path C" (React + headless renderer both driving one shared `StagingBuffer` and one shared `ObservableDataModel`) without hand-rolling the reconciliation, field-ID validation, or staging-aware action resolution pieces.
  ...
  - **`validateUniqueFieldIds(tree)`** + `DuplicateFieldIdError` — new. Walks a `UITree` and throws `DuplicateFieldIdError` when two elements carry the same `id` prop. Returns the set of discovered IDs on success. Implements NC's Invariant 8 (field-ID uniqueness) at the tree-emission boundary. The error names both colliding element keys so the caller can point the user at the offending spot without re-walking.
  - **`resolve-with-staging.ts` — shared helpers for staging-aware resolution.** Extracts the "staging wins for single-segment paths, data wins otherwise" rule used by the headless renderer into a set of reusable functions: `resolveStagingOrDataPath(path, staging, data)`, `isDynamicPathLiteral(v)`, `preResolveDynamicParams(params, staging, data)`, and `resolveActionWithStaging(action, staging, data)`. The headless package's `context.ts` now delegates to `resolveActionWithStaging` instead of reimplementing the rule inline. NC's React path consumes the same helper directly, so there is exactly one implementation of the rule across the whole repo. The helper also fixes a subtle bug: `confirm.variant`, `confirmLabel`, and `cancelLabel` now pass through explicitly rather than relying on an object spread.
  - **Test coverage:** 28 new tests across 3 new test files (6 `collect-field-ids.test.ts` + 7 `validate-field-ids.test.ts` + 20 `resolve-with-staging.test.ts`, minus 6 moved from headless). Full repo test count: 415 → 443.

- **`@json-ui/react` `StagingProvider` + hooks** — new React binding for Plan 1's `StagingBuffer` primitive. Mirrors the `DataProvider` external-store mode that binds `ObservableDataModel`, so NC's Path C has a symmetric pair of React providers for the two observable stores.
  - **`StagingProvider`** — accepts an optional `store?: StagingBuffer` prop. Binds via `useSyncExternalStore` in both modes (internal mode creates its own session-lifetime buffer via `createStagingBuffer()`; external mode binds to the caller-provided buffer). Unlike `DataProvider`, the two modes use the same hook shape, so no split-component dispatcher is needed.
  - **`useStaging()`** — returns the full context value: `snapshot`, `getField`, `hasField`, `setField`, `deleteField`, `reconcile`, and `reconcileAgainstTree` — the convenience wrapper that walks a `UITree` via `collectFieldIds` and calls `buffer.reconcile` with the result. NC's renderer wrapper will call `reconcileAgainstTree` after a successful LLM tree commit (NC Invariant 9 — partial-tree safety).
  - **`useStagingField<T>(fieldId)`** — returns a `[value, setValue]` tuple shaped like `useState`. The primary hook NC input components (`TextField`, `Checkbox`, etc.) will consume.
  - **`useStagingSnapshot()`** — read-only full-snapshot accessor. Used by the NC renderer wrapper at intent-flush time to capture the snapshot for the `IntentEvent` payload.
  - **Test coverage:** 14 new tests. Internal mode (create + read + write + delete + reconcile + `reconcileAgainstTree`), external mode (bind to an existing buffer, external writes propagate to React, React writes propagate to the buffer, two providers sharing one buffer stay in sync — the primary Path C use case), `useStagingField` binding semantics, `useStaging` error-out-of-provider path. Full repo test count: 443 → 457.
